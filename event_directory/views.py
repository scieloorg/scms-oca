...
from .permission_helper import EventDirectoryPermissionHelper
...
class EventDirectoryEditView(EditView):
    ...
    def must_moderate(self):
        ...
        return EventDirectoryPermissionHelper(model=self.model).must_be_moderate(
            self.request.user
        )
...
def validate(request):
    """
    Validate CSV file for EventDirectory import.
    """
    format_file_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "chkcsvfmt.fmt"
    )
    return validate_directory_file(request, EventDirectoryFile, format_file_path)
    ...
    try:
        with open(file_path, "r") as csvfile:
            ...
            for line, row in enumerate(data):
                ...
                di = get_directory_instance(EventDirectory, record_id)

                set_common_fields(di, row, request.user, action_filter="disseminação")
                get_dates_and_times(di, row)
                di.attendance = get_row_value(row, "Attendance")
                di.save()
                ...
                sync_keywords(di.keywords, row.get("Keywords"))
...
def download_sample(request):
    """
    Download CSV sample file for EventDirectory import.
    """
    sample_file_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "example_event.csv"
    )

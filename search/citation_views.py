...
from .citation_constants import CITATION_PRESET_STYLES
...
from .citeproc_render import list_installed_csl_styles
...
def _file_attachment(content, mime, ext):
    ...
    resp["Content-Disposition"] = f'attachment; filename="citation.{ext}"'
    return resp
...
def _handle_citation_errors(view_fn):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        except CitationBadRequest as err:
...
def citation_custom_style_view(request):
    ...
    if not style:
        raise CitationBadRequest(_("Style name is required."))
    inputs = extract_preview_inputs(body, request)

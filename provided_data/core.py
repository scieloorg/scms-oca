...
class Name(models.Model):
    ...
    type = models.CharField(
        _("Type"), choices=NAME_TYPES, max_length=20, null=True, blank=True
    )
...
class Authorship(models.Model):
    ...
    citation_names = models.ManyToManyField(
        "Name", verbose_name=_("Citation name"), related_name="+", blank=True
    )
    person_research_areas = models.ManyToManyField(
        "ResearchArea", verbose_name=_("Research area"), related_name="+", blank=True
    )
    ...
    def data(self):
        d = {
            ...
            "authorship__citation_names": [
                c.data for c in self.citation_name.iterator()
            ],
            "authorship__person_research_areas": [
                r.data for r in self.research_area.iterator()
            ],
    ...
    def authorship_get_or_create(
        ...
        try:
            ...
            for citation_name in citation_names or []:
                authorship.citation_names.add(
                    Name.create(text=citation_name, type="citation name")
                )
            for research_area in person_research_areas or []:
                authorship.person_research_areas.add(
                    ResearchArea.get_or_create(text=research_area)
                )
            ...
            authorship.birth_country = Country.get_or_create(
                user=user, name_pt=birth_country, name_en=birth_country
            )
            authorship.save()
...
class CommonPublicationData(CommonControlField):
    ...
    keywords = models.ManyToManyField(
        "CommonTextField", verbose_name=_("Keywords"), blank=True
    )
    document_titles = models.ManyToManyField(
        "CommonTextField", verbose_name=_("Titles"), related_name="+", blank=True
    )
    authors = models.ManyToManyField(
        "Authorship", verbose_name=_("Authors"), blank=True
    )
    publication_date = models.CharField(
        _("Publication Date"), max_length=10, null=True, blank=True
    )
    ...
    def __unicode__(self):
        return " | ".join(
            str(document_title) for document_title in self.document_titles
        )
    ...
    def data(self):
        return {
            ...
            "common_publication_data__keywords": [
                k.data for k in self.keywords.iterator()
            ],
            "common_publication_data__document_titles": [
                t.data for t in self.document_titles.iterator()
            ],
            "common_publication_data__authors": [
                a.data for a in self.authors.iterator()
            ],
            ...
            "common_publication_data__research_areas": [
                r.data for r in self.research_areas.iterator()
            ],
    ...
    def get_or_create(
        ...
        try:
            ...
            for document_title in document_titles or []:
                common_publication.document_titles.add(
                    CommonTextField.create(document_title)
                )
            ...
            for research_area in research_areas or []:
                common_publication.research_areas.add(
                    ResearchArea.get_or_create(text=research_area)
                )

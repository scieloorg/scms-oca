...
def normalize_option(option, selected_values):
    ...
    normalized_label = gettext(label) if isinstance(label, str) and label else (label or normalized_value)

    return {
        "value": normalized_value,
        "label": normalized_label,
        "group": gettext(group) if isinstance(group, str) and group else group,
